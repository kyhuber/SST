...
export async function fetchFunds(passphrase: string): Promise<FundsSnapshot> {
  const response = await fetch(`${WORKER_URL}/api/funds`, {
  ...
  return (await response.json()) as FundsSnapshot;
}
